...
if (match_token(parser, TOKEN_STRING)) {
    ...
    memcpy(value, token.start + 1, token.length - 2);
...
if (match_token(parser, TOKEN_IDENTIFIER)) {
    ...
    AstNode *node = ast_variable(name);
...
if (match_token(parser, TOKEN_OTHERWISE)) {
    if (check_token(parser, TOKEN_CHECK)) {
        ...
        while (
            ...
            ast_program_add(otherwise, statement);
...
if (!match_token(parser, TOKEN_DEDENT)) {
    parser_error(parser, "expected end of during block");
    ast_free(node);
    return NULL;
}
