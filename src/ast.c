...
AstNode *ast_run(AstNode *command)
{
    AstNode *node = create_node(AST_RUN);
...
AstNode *ast_list(void)
{
    return create_node(AST_LIST);
}
...
)
{
    ...
    switch (node->type) {
        ...
        case AST_RUN:
            printf("Run\n");
            ast_print(node->run.command, indent + 1);
            break;
        ...
        case AST_LIST:
            ...
            for (size_t i = 0; i < node->list.count; i++)
                ast_print(node->list.items[i], indent + 1);
            break;
